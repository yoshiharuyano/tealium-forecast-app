...
uploaded_file = st.file_uploader('過去の利用データをアップロード（CSV）', type=['csv'])

if uploaded_file:
    # CSV形式をShift_JISで読み込み（日本語対応）
    ...
    # 繁忙期設定
    st.subheader('繁忙期の設定')
    busy_months = st.multiselect('繁忙期の月を選択', options=list(range(1, 13)), default=[3, 4, 9, 10, 12])
