# Streamlitのインストールが必要な場合は以下を実行してください。
...
# ファイルアップロード
uploaded_file = st.file_uploader('過去の利用状況データをアップロードしてください（Excel形式）')

if uploaded_file is not None:
    df = pd.read_excel(uploaded_file, sheet_name='osakagas_daily_20240401_2025033')

    # 日付処理
    df['日付'] = pd.to_datetime(df.iloc[:, 0], errors='coerce')
    ...
    st.subheader('予測結果')
    st.dataframe(forecast_df[['日付', '予測セッション', '予測イベント']])
